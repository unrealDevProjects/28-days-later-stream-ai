<script lang="ts">
  ...
  // Ajustar throttle - 30fps es suficiente para tiempo real fluido
  // 1000/30 = ~33ms entre frames
  const THROTTLE = 1000 / 30; // 30fps para mejor rendimiento
...
<div
  class="relative mx-auto w-full overflow-hidden rounded-lg border-4 border-slate-400 bg-slate-100"
>
  <div class="relative z-10 aspect-video w-full">
    ...
    <video
      ...
      loop
    ></video>
    <canvas bind:this={canvasEl} class="absolute left-0 top-0 h-full w-full object-cover"></canvas>
  </div>
  <div class="absolute left-0 top-0 flex aspect-video w-full items-center justify-center">
