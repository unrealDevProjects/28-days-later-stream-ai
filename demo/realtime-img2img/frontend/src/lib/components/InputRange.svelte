<script lang="ts">
  ...
  // Detectar cuando el usuario interactúa directamente con el slider
  function handleUserInput() {
    // Solo para denoise_strength, emitir evento cuando el usuario interactúa
    if (params.id === 'denoise_strength') {
      dispatch('userInput');
    }
  }
</script>
...
<div class="grid max-w-md grid-cols-4 items-center gap-3">
  ...
  <input
    ...
    max={params?.max}
    step={params?.step ?? 1}
    on:mousedown={handleUserInput}
    on:touchstart={handleUserInput}
